...
%
% PP  = CSAPE(X,Y) は、ラグランジュ (Lagrange) 端条件 (下表のデフォルト)を
% 使って、与えられたデータ (X,Y) への 3 次スプライン補間 (pp-型) を
% 出力します。補間は、データ位置 X(j) で、与えられたデータ値 Y(:,j), 
% j=1:length(X) と一致します。データ値は、スカラ、ベクトル、行列、または
% N次元配列です。同じサイトのデータ点は、平均化されます。
% グリッド化されたデータの補間については、以下を参照してください。
%
% PP  = CSAPE(X,Y,CONDS) は、端条件の値 endcondvals に対応する CONDS 
% で指定された端条件を使います。
% データ値がデータ位置より 2 つ多い場合、最初 (最後) のデータ値は、左側
% (右側)の端条件に対する値として得られます。すなわち、
...
%                 を作成します(もしある場合は、与えられた端条件の値を
%                 無視します)。
% 'periodic'    : 最初の点での1次および2次微分を最後の点のものと一致
%                 させます。
%                 (もしある場合は、与えられた端条件の値を無視します)
...
% されることを意味します。CONDS(1)=0=CONDS(2) は、周期的端条件を意味
% します。
%
% CONDS(i) は、設定されていないか、または 0, 1, 2 と異なる場合、CONDS(i) 
% のデフォルト値は1になり、endcondvals(:,i) のデフォルト値が設定されます。
% 端条件の値が指定されなければ、endcondvals(:,i) のデフォルト値は、
% 以下になります。
%
%    CONDS(i)=1 ならば、最近接の 4 点に対する 3 次補間の微分
%    CONDS(i)=2 ならば、0
%
...
% 手順と一致し、さらに1変数の端条件のテンソル積は、与えられた値と共に
...
%
% 多変数の設定における端条件の指定について説明します。これは2変数3次
...
% 周期的な端条件を使ったパラメトリック2変数 3 次スプラインと
% して、球面を考えます。
%
