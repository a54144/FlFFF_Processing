% CSAPI   節点でない端点条件をもつ 3 次スプライン補間
%
% PP  = CSAPI(X,Y)  は、節点ではない端点条件を使って与えられたデータ 
% (X,Y) への (pp-型の) 3 次スプライン補間を出力します。
% 補間は、データ位置 X(j) で、与えられたデータ値 Y(:,j), j=1:length(X) 
...
% データ位置 (X1(j1), ..., Xm(jm)) でデータ値 Y(:,j1,...,jm) に一致
% する m 点の (pp-型の) 3 次スプライン補間を出力します。
