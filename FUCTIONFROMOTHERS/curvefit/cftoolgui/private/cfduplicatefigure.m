...
%   $Revision: 1.1.6.10 $    $Date: 2012/08/20 23:53:21 $
...
%-----------------------------------------------------------------------
    function nPlotFit( ft )
        ...
        if doShowBounds
            ...
            boundDisplayName = sprintf('%s%s', ...
                BOUND_INDENT, ...
                getString(message('curvefit:cftoolgui:PredBnds', ft.name )));
...
% -- The bounds get indented (from the fits) if there are two or more
% fits
% --> Thus if the given database has two or more record we return a
