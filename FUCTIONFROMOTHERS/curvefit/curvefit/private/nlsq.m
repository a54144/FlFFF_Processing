...
%   $Revision: 1.12.2.11 $  $Date: 2012/08/20 23:55:43 $
...
%         SD=-(GRAD*GRAD'+(norm(GRAD)+1)*(eye(numberOfVariables,numberOfVariables)))\(GRAD*CostFunction);
      ...
      elseif NewF<FIRSTF
         ...
         how='Red_Step';
         if NewF==FIRSTF,
            msg = getString(message('curvefit:curvefit:FunctionValueConverged'));     
            status=1;
            EXITFLAG = 1;
         else
   ...
   % Check Termination 
   if max(abs(SD))< tolX 
       msg = getString(message('curvefit:curvefit:SearchDirectionLessThanTolX'));
       status=1; EXITFLAG=1;
   elseif (GradF'*SD) < tolFun && ...
           max(abs(GradF)) < 10*(tolFun+tolX)
       msg = getString(message('curvefit:curvefit:GradientInTheSearchDirectionLessThanTolFun'));
       status=1; EXITFLAG=1;
       
   elseif numFunEvals > maxFunEvals
       msg = getString(message('curvefit:curvefit:MaxNumFunctionEvaluationsExceeded'));
       status=1;
       EXITFLAG = 0;
   elseif iter > maxIter
       msg = getString(message('curvefit:curvefit:MaxNumIterationsExceeded'));
       status=1;
       EXITFLAG = 0;
   else
