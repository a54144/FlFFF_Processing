...
%   $Revision: 1.11.2.17 $  $Date: 2012/08/20 23:55:42 $
...
try
    ...
    lsqnException = badFunctionException(fevalException);
    throw( lsqnException );
end
...
function lsqnException = badFunctionException(fevalException)
% badFunctionException   An MException created for a "bad" user function.

% Throw an exception with the computed message, but also pass along the original
% cause of the exception to the caller.
lsqnException = curvefit.exception( 'curvefit:lsqncommon:userFcnFailed', fevalException.message);
