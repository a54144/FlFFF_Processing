function [Factors,it,err,corcondia]=FittingbyYY(X,Fac,Options,const,OldLoad,FixMode,Weights);

%    This file is basically same to parafac.m. Just replacing the text from
%    'parafac' to 'FittingbyYY' (Youhei Yamashita, 29Oct2008).



% FittingbyYY multiway FittingbyYY model
%
% See also:
...
%
%                  THE FittingbyYY MODEL
%     ___________________________________________________
%
% [Factors,it,err,corcondia] = FittingbyYY(X,Fac,Options,const,OldLoad,FixMode,Weights);
%
% or skipping optional in/outputs
%
% Factors = FittingbyYY(X,Fac);
%
% Algorithm for computing an N-way FittingbyYY model. Optionally
% constraints can be put on individual modes for obtaining
% orthogonal, nonnegative, or unimodal solutions. The algorithm
% also handles missing data. For details of FittingbyYY
...
%            This option is ignored if FittingbyYY is started with old values.
...
% ---------------------OUTPUT---------------------
%
% Factors    FittingbyYY estimate of loadings in one matrix. For a 3 component
...
if nargin==0
  disp(' ')
  disp(' ')
  disp(' THE FittingbyYY MODEL')
  disp(' ')
  disp(' Type <<help FittingbyYY>> for more info')
  disp('  ')
  disp(' [Factors,it,err,Corcondia] = FittingbyYY(X,Fac,Options,const,OldLoad,FixMode,Weights);')
  disp(' or short')
  disp(' Factors = FittingbyYY(X,Fac);')
  disp(' ')
  disp(' Options=[Crit Init Plot NotUsed ShowFit MaxIt]')
  disp(' ')
  disp(' ')
  disp(' EXAMPLE:')
  disp(' To fit a four-component FittingbyYY model to X of size 6 x 2 x 200 x 3 type')
  disp(' Factors=FittingbyYY(X,4)')
...
elseif Init==10
  ...
  Opt(2) = 0;
  ERR=[];
  [Factors,it,err] = FittingbyYY(reshape(X,DimX),Fac,Opt,const,[],[],Weights);
  ERR = [ERR;err];
  Opt(2) = 1;
  [F,it,Err] = FittingbyYY(reshape(X,DimX),Fac,Opt,const,[],[],Weights);
  ERR=[ERR;Err];
  if Err<err
    Factors=F;
    err=Err;
  ...
  error(' Problem in FittingbyYY initialization - Not set correct')
end
...
if showfit~=-1 & DimX(1)>1
  ...
  if max(max(abs(phi)-diag(diag(phi))))>.85
    ...
    disp(' ')
    disp(' - Or a FittingbyYY with some modes collapsed (if # modes > 3)')
    disp(' ')
  end
end
...
if length(loads)==order % FittingbyYY
