...
addpath('C:\Matlab Processing\FlFFF Data\RAWDATA/'); 
...
MW_Calibration_Path = 'C:\Matlab Processing\FlFFF Data\TIMECALIBRATED/Calibration of D to MW using PSS Standards/20150101/';
...
Table_relation_DC_MW = load(strcat(MW_Calibration_Path,'Table_relation_DC_MW.dat'));
...
%     save FFF_Measurement_time.dat FFF_Measurement_time -ascii -tabs;
    ...
    for detectorr = 1: size(Detectors_want_to_plot,1);
        ...
        Slope_Relation_DC_MW = Table_relation_DC_MW(1,detectorr)
        Intercept_Relation_DC_MW = Table_relation_DC_MW(2,detectorr)
% % % % % % % % % %         for i = 1:size(Real_D_H,1);
% % % % % % % % % %             Real_W_M(i) = 10^((log10(Real_Diff_Coe(i)^(-1))-7.964594797)/0.525643121);%10^((log10(Real_Diff_Coe(i)^(-1))-8.363275506)/0.376487071);%
% % % % % % % % % %         end;
        for i = 1:size(Real_Diff_Coe,1);
            Real_W_M(i) = 10^((log10(Real_Diff_Coe(i)^(-1))-Intercept_Relation_DC_MW)/Slope_Relation_DC_MW);%10^((log10(Real_Diff_Coe(i)^(-1))-7.964594797)/0.525643121)
        end;
        
        Real_Data = Data_tube_time_corrected(number_Selected_Detector:size(Data_tube_time_corrected,1));
 
        if If_some_specific_sample_needs_signal_manipulation == 1;
            if (strcmp(filename,'HaborMixed-sur-10012014_1ml_Carrier_NaCl_10mM_H3BO3_5mM_pH8_11242014.dat')==1) & (detectorr ==4);
                time_beiju_01 = 67.9;%%%%%%%%%%64.5%65.04;%%%%%%%68%68.1%68
                time_beiju_02 = 68.1;%%%%%%%%%%%%%66%65.24;%%%%%%%68.2%68.2%68.2
                time_savior_01= 69;%%%%%%%69;
                time_savior_02= 70;%%%%%%%70;
                time_specific_this_sample = [time_beiju_01;time_beiju_02;time_savior_01;time_savior_02];
                Transition_time_in_minute_s = Transition_time_in_minute*ones(size(time_specific_this_sample));
                Tube_Time_minute_s = Tube_Time_minute*ones(size(time_specific_this_sample));
 
                time_specific_this_sample=(time_specific_this_sample-Transition_time_in_minute_s-Tube_Time_minute_s).*60;
                Real_Data (Time_tube_time_corrected>time_specific_this_sample(1) & Time_tube_time_corrected<time_specific_this_sample(2)) =  mean(Real_Data (Time_tube_time_corrected>time_specific_this_sample(3) & Time_tube_time_corrected<time_specific_this_sample(4)))*ones(size(Real_Data (Time_tube_time_corrected>time_specific_this_sample(1) & Time_tube_time_corrected<time_specific_this_sample(2))));
            end;
        end;
